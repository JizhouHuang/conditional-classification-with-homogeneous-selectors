...
from ..utils.data import MultiLabeledDataset, FixedIterationLoader
...
class SelectiveHalfspaceLearner(nn.Module):
    ...
    def data_split(
        ...
        self.lr *= torch.sqrt(
            1/torch.tensor(self.num_iter * dataset.num_features())
        )
        print(f"{self.header} learning rate is {self.lr}")
    ...
    def PSGDOptim(
            ...
            dataset_train: Union[Subset, MultiLabeledDataset],
            dataset_sel_pgd: Union[Subset, MultiLabeledDataset] = None
    ) -> LinearModel:
        """
        ...
        dataloader_train = FixedIterationLoader(
        ...
        if dataset_sel_pgd is not None:
            lin_model = self.psgd_with_model_selection(
                lin_model=lin_model,
    ...
    def psgd(
        ...
        # features: [num train sample, num features]
        for labels_train, features_train in dataloader_train:
            # update weights
            self.grad_update(
                lin_model=lin_model,        # [num observations, num predictors, num features]
                labels=labels_train.t(),    # [num predictors, num train sample]
                features=features_train     # [num train sample, num features]
            )
        
        return lin_model
    
    def psgd_with_model_selection(
            self,
            lin_model: LinearModel,
            dataloader_train: FixedIterationLoader,
        ...
        # labels:   [num predictors, num train sample]
        # features: [num train sample, num features]
        labels_sel, features_sel = dataset_sel[:]
        for labels_train, features_train in dataloader_train:
            # update weights
            self.grad_update(
                lin_model=lin_model,        # [num observations, num predictors, num features]
            ...
            conditional_error_rates = lin_model.conditional_one_rate(
                y=labels_sel.t(),           # [num predictors, num sel sample]
                X=features_sel              # [num sel sample, num features]
    ...
    def grad_update(
        ...
        self.converged_bar.n = int((torch.norm(proj_grads, p=2, dim=-1) < 0.075).sum())
